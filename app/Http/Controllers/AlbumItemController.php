...
class AlbumItemController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $count = ItemImage::where('company_id', $cid)->get()->count();
    // $count = count($images);
    $total_bytes_value = ItemImage::where('company_id', $cid)->sum('size');
    // 画像総容量をギガバイト数に変換
  ...
  // }
  public function destroy(ItemImage $itemimage, Request $request)
  {
    $user = Auth::user();
    $cid = $user->company_id;
    ...
    foreach ($img_id as $img) {
      ...
      $img_name = $itemimage->filename;
      // $img_name = Str::afterLast($img, '/');
      // 最後のスラッシュより後の文字列を取得。ファイル名取得
      $path_as = $cid . '/items/' . $img_name;
